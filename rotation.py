import streamlit as st
  ...
  st.markdown("""
      <style>
      .meal-bg { background-color: #ffff00 !important; color: black !important; font-weight:
  bold; }
      </style>
  ...
  # 🔗 매장 및 시트 설정
  STORES = {
      "하우스 서울": {"ID": "19CvEiqbhPqNpz2KzcBQh7vVaH40O_ZuR6MFYdw98c5Q", "TO_GID":
  "2126973547"},
      "하우스 도산": {"ID": "1nqSbhCPnO1o_vRSubJCuLjbbZxmtRMjioTtA_ZzzNLc", "TO_GID":
  "2126973547"},
      ...
      for _, row in data.iterrows():
          ...
          if name and any(kw in stype for kw in ['정직', '파트']):
              res.append({
                  ...
                  "can_counter": any(x in str(row.get('카운터여부', 'X')).lower() for x in ['o',
  'y', '1', 'v', '예'])
              })
      return res
  ...
  selected_pt_names = st.sidebar.multiselect("⏱️ 출근 파트타이머 선택", [s['original_name'] for s
  in pt_list], default=[s['original_name'] for s in pt_list])
  ...
  if selected_pt_names:
      ...
      for pt_name in selected_pt_names:
          ...
          with st.sidebar.expander(f"👤 {pt_name}"):
              c1, c2 = st.columns(2)
              new_in = c1.text_input(f"출근", value=pt_origin['in'] or "11:00",
  key=f"in_{pt_name}")
              new_out = c2.text_input(f"퇴근", value=pt_origin['out'] or "21:00",
  key=f"out_{pt_name}")
              new_meal = st.text_input(f"식사", value=pt_origin['meal_p'] or "12:00",
  key=f"meal_{pt_name}")
  ...
  def run_rotation():
      ...
      for slot in all_time_slots:
          ...
          if not to_row.empty:
              ...
              sorted_active_zones = sorted(active_zones, key=lambda z: (get_zone_priority(z),
  all_zones.index(z)))
  ...
  if 'result_df' in st.session_state:
      ...
      html = "<table style='width:100%; border-collapse: collapse; text-align: center; border: 1px
  solid #ddd;'>"
      html += "<tr style='background-color: #f8f9fa;'><th style='border: 1px solid #ddd; padding:
  10px;'>시간</th>"
      for name in edited_df.columns:
          s_info = next((s for s in final_staff_configs if s['display_name'] == name), None)
          color = "#007bff" if s_info and s_info["type"] == '정직' and s_info["in"] <= "10:00"
  else "#e83e8c"
          html += f"<th style='border: 1px solid #ddd; padding: 10px; color: {color}; font-weight:
  bold;'>{name}</th>"
      html += "</tr>"
      for slot, row in edited_df.iterrows():
          html += f"<tr><td style='border: 1px solid #ddd; padding: 8px; font-weight:
  bold;'>{slot}</td>"
          for val in row:
