...
st.title("🕶️ GENTLE MONSTER 전사 통합 로테이션 (v43.0)")
...
db_df = load_db()
if db_df.empty:
    st.error("❌ 시트 로드 실패: 데이터가 비어있거나 시트 ID/공유설정을 확인하세요."); st.stop()
...
def extract_names(data, keyword):
    ...
    # 열 이름이 정확히 일치하지 않아도 키워드가 포함된 열을 찾음
    ...
    if not type_col or not name_col:
        # 열 이름을 못 찾으면 데이터 구조상 2번째(이름), 3번째(구분) 열 시도 (안전장치)
        try:
            name_col = data.columns[1]
            type_col = data.columns[2]
        except: return []
...
for i, ft in enumerate(working_ft):
    ...
    if not match.empty:
        ...
        try:
            s_v = int(float(r.get('출근시간', 10)))
            e_v = int(float(r.get('퇴근시간', 20)))
        except: pass
...
# 5. 로테이션 알고리즘 (X방지, 우선순위, 2시간 연속 금지)
def generate_v43():
    ...
    for slot in time_slots:
        ...
        for z in target_zones:
            ...
            for _ in range(max_to):
                if not pool: break
                candidates = [p for p in pool if last_zone[p] != z]
...
if st.sidebar.button("🚀 로테이션 생성"):
    st.session_state.df_v43 = generate_v43()

if 'df_v43' in st.session_state:
    st.subheader(f"📊 {selected_store} 스케줄표 (매니저별)")
    st.data_editor(st.session_state.df_v43, use_container_width=True, height=600)
